...
@property (strong, nonatomic) IBOutlet UISegmentedControl *queSeg;
...
- (void)viewDidLoad {
    ...
    //设置两个tableView的属性 44
    self.askView=[[UIView alloc]initWithFrame:CGRectMake(0, 44, self.view.frame.size.width, self.view.frame.size.height-108)];
    self.acceiptTabView=[[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-108)];
    self.askTabView=[[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-108)];
    self.acceiptView=[[UIView alloc]initWithFrame:CGRectMake(0, 44, self.view.frame.size.width, self.view.frame.size.height-108)];
    self.doneTabView=[[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-108)];
    self.doneView=[[UIView alloc]initWithFrame:CGRectMake(0, 44, self.view.frame.size.width, self.view.frame.size.height-108)];
...
-(void)tapSeg
{
    switch (self.queSeg.selectedSegmentIndex) {
        ...
        case 1:
            ...
            [self.doneTabView removeFromSuperview];
...
-(void)accTableheaderRereshing {
    ...
    [RequestServer fetchMethodName:@"myQuestions" parameters:@{@"userId":userId,@"userAccount":userAccount,@"userPwd":userPwd,@"status":@"2",@"pageNum":[NSString stringWithFormat:@"%d",self.accCurrentPage],@"pageSize":@"10"} shouldDisplayLoadingIndicator:NO puhuiRequestType:PuhuiJK successHandler:^(NSMutableDictionary *responseDic) {
        ...
        for (NSDictionary *dict  in inforArys) {
            QuestionModel *question=[QuestionModel objectWithKeyValues:dict];
            if ([question.status isEqualToString:@"1"]||[question.status isEqualToString:@"2"]) {
