...
@interface FriendConmendController () <UIAlertViewDelegate,UITableViewDataSource,UITableViewDelegate,MFMessageComposeViewControllerDelegate>
...
@implementation FriendConmendController
//懒加载
//*模型数据*/
...
{
    if (!_scroll) {
        _scroll = [[UIScrollView alloc] initWithFrame:CGRectMake(8, 48, self.view.frame.size.width-16, self.view.frame.size.height-54)];
...
{
    if (!_historyView) {
        _historyView = [[UIView alloc] initWithFrame:CGRectMake(0, 48, self.view.frame.size.width, self.view.frame.size.height-54)];
    }
    return _historyView;
}
